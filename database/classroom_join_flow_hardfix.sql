-- Nexo 11 - Hardfix do fluxo "Entrar com código" da Sala de Aula
-- Corrige funções antigas com ambiguidade (42702) e estabiliza:
-- - criar sala
-- - preview por código
-- - solicitar entrada
-- - sair da sala
...
-- Remove duplicidades legadas antes dos índices únicos.
with ranked as (
  select
    cm.id,
    row_number() over (
      partition by cm.classroom_id, cm.profile_id
      order by cm.updated_at desc nulls last, cm.created_at desc nulls last, cm.id desc
    ) as rn
  from public.classroom_members cm
)
delete from public.classroom_members d
using ranked r
where d.id = r.id
  and r.rn > 1;
...
with ranked as (
  select
    q.id,
    row_number() over (
      partition by q.classroom_id, q.student_profile_id, q.status
      order by q.requested_at desc nulls last, q.updated_at desc nulls last, q.id desc
    ) as rn
  from public.classroom_join_requests q
  where q.status = 'requested'
)
delete from public.classroom_join_requests d
using ranked r
where d.id = r.id
  and r.rn > 1;
...
create index if not exists classroom_members_profile_status_idx
...
create index if not exists classroom_join_requests_classroom_status_idx
...
begin
  loop
    ...
    if v_try >= 100 then
      raise exception 'Nao foi possivel gerar codigo unico para sala';
    end if;
  end loop;
end;
$$;
...
begin
  ...
  if v_request_id is not null then
    update public.classroom_join_requests q
       set requested_at = timezone('utc', now()),
           updated_at = timezone('utc', now())
...
begin
  ...
  if v_classroom.id is null then
    return query
    select false, p_classroom_id, 'Sala nao encontrada.'::text;
    return;
  end if;
  ...
  delete from public.classroom_members cm
   where cm.classroom_id = v_classroom.id;

  delete from public.classrooms c
   where c.id = v_classroom.id;
