...
select
  'Debug de Algoritmo',
  'Corrigir um algoritmo com bug e explicar o ajuste.',
  'course',
  'informatica',
  14
where not exists (
  select 1 from public.learning_activities where title = 'Debug de Algoritmo'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'SQL Rapido',
  'Criar uma consulta com filtro, ordenacao e agrupamento.',
  'course',
  'informatica',
  14
where not exists (
  select 1 from public.learning_activities where title = 'SQL Rapido'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Mini API',
  'Modelar um endpoint simples e descrever entrada e saida.',
  'course',
  'informatica',
  16
where not exists (
  select 1 from public.learning_activities where title = 'Mini API'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'HTML + CSS',
  'Montar um card responsivo com HTML e CSS.',
  'course',
  'informatica',
  12
where not exists (
  select 1 from public.learning_activities where title = 'HTML + CSS'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Rede Local',
  'Explicar a configuracao basica de rede para um laboratorio.',
  'course',
  'informatica',
  12
where not exists (
  select 1 from public.learning_activities where title = 'Rede Local'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Versionamento Git',
  'Criar fluxo simples com branch, commit e merge.',
  'course',
  'informatica',
  13
where not exists (
  select 1 from public.learning_activities where title = 'Versionamento Git'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Planejamento de Projeto',
  'Definir backlog curto com prioridades da entrega.',
  'base',
  null,
  10
where not exists (
  select 1 from public.learning_activities where title = 'Planejamento de Projeto'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Leitura Tecnica',
  'Ler um texto da disciplina e resumir em 5 pontos.',
  'base',
  null,
  9
where not exists (
  select 1 from public.learning_activities where title = 'Leitura Tecnica'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Problema em Dupla',
  'Resolver um problema pratico em dupla com justificativa.',
  'base',
  null,
  12
where not exists (
  select 1 from public.learning_activities where title = 'Problema em Dupla'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Revisao de Unidade',
  'Criar mapa mental com os topicos principais da unidade.',
  'base',
  null,
  10
where not exists (
  select 1 from public.learning_activities where title = 'Revisao de Unidade'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Desafio de Logica',
  'Responder desafio curto de logica e explicar a estrategia.',
  'base',
  null,
  11
where not exists (
  select 1 from public.learning_activities where title = 'Desafio de Logica'
);

insert into public.learning_activities (title, description, activity_type, course_area, xp_reward)
select
  'Apresentacao Expressa',
  'Apresentar em 3 minutos a solucao da atividade do dia.',
  'base',
  null,
  10
where not exists (
  select 1 from public.learning_activities where title = 'Apresentacao Expressa'
);
...
declare
  ...
  v_activity_id uuid;
begin
  select coalesce(course_area, 'base_central')
  into v_course_one
  from public.profiles
  where id = p_profile_one_id;

  select coalesce(course_area, 'base_central')
  into v_course_two
  from public.profiles
  where id = p_profile_two_id;
  ...
  if v_use_course then
    ...
    where is_active = true
      ...
      and course_area = v_target_course
      and (p_prev_activity_id is null or id <> p_prev_activity_id)
    order by random()
    limit 1;
...
create or replace function public.academy_ensure_pet_pair(
...
begin
  ...
  insert into public.chat_pet_pairs (conversation_id)
...
$academy_ensure_pet_pair$;

grant execute on function public.academy_ensure_pet_pair(uuid) to authenticated;
...
begin
  ...
  v_pair_id := public.academy_ensure_pet_pair(p_conversation_id);
...
$academy_realtime_pet_pairs$;

do $academy_realtime_pet_tasks$
begin
  if not exists (
    select 1
    from pg_publication_tables
...
$academy_realtime_pet_tasks$;

do $academy_realtime_pet_completions$
begin
  if not exists (
    select 1
    from pg_publication_tables
...
$academy_realtime_pet_completions$;
