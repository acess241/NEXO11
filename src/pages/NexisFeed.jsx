...
function NexisVideo({ item, ativo, onMediaError }) {
  ...
  return <video ref={ref} src={item.playUrl || item.media_url} loop playsInline preload="metadata" onClick={(event) => event.currentTarget.paused ? event.currentTarget.play() : event.currentTarget.pause()} onError={() => onMediaError(item)} />
}
...
export default function NexisFeed() {
  ...
  async function carregar() {
    ...
    try {
      ...
      const [perfisResp, likesResp, commentsResp] = await Promise.all([
      ...
      const perfis = new Map((perfisResp.data || []).map((perfilItem) => [perfilItem.id, perfilItem]))
      const finais = (posts || []).filter((post) => {
        ...
        comentarios: (commentsResp.data || []).filter((item) => item.post_id === post.id),
      }))
      setItens(finais)
  ...
  async function publicarComentario(event) {
    ...
    if (error) return setErro(error.message)
    setItens((prev) => prev.map((item) => item.id === comentando.id ? { ...item, comentarios: [...item.comentarios, data] } : item))
    setComentando((prev) => ({ ...prev, comentarios: [...prev.comentarios, data] }))
    setComentario('')
  }
  ...
  return (
    <div className="nexis-page">
      <header className="nexis-topbar"><strong>NEXIS</strong><button type="button" onClick={() => navigate('/novo-post')}>Criar</button></header>
      {erro ? <div className="nexis-error">{erro}<button onClick={() => setErro('')}>×</button></div> : null}
      <main className="nexis-scroll" ref={containerRef}>
        {itens.map((item) => (
          <article className="nexis-reel" data-id={item.id} key={item.id}>
            <NexisVideo item={item} ativo={ativoId === item.id} onMediaError={recuperarMidia} />
      ...
      {comentando ? (
        <div className="nexis-comments-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setComentando(null)}>
          <section className="nexis-comments-sheet">
            ...
            <div>{comentando.comentarios.map((item) => <p key={item.id}>{item.content}</p>)}{!comentando.comentarios.length ? <small>Seja o primeiro a comentar.</small> : null}</div>
