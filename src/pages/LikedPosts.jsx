...
export default function LikedPosts() {
  ...
  async function carregar() {
    try {
      ...
      if (likesResp.error) {
        likesResp = await supabase.from('post_likes').select('post_id').eq('profile_id', perfil.id).limit(120)
      }

      if (likesResp.error) throw likesResp.error

      const idsPosts = [...new Set((likesResp.data || []).map((item) => item.post_id).filter(Boolean))]
      ...
      setErro('Não foi possível carregar seus curtidos agora.')
    } finally {
      setCarregando(false)
    }
  }
  ...
  return (
    <div className="container">
      ...
      <div className="page settings-page">
        <p className="settings-kicker">Publicações que você curtiu</p>
            ...
            {itens.map((item) => (
              ...
              >
                <div className="settings-list-avatar">
                  {item.autor?.foto_url ? (
                    <img src={item.autor.foto_url} alt={formatDisplayName(item.autor.nome) || item.autor.username} />
                  ) : (
                ...
                <div className="settings-list-copy">
                  ...
                  <small>
                    {item.totalCurtidas} curtida(s) - {item.totalComentarios} comentário(s)
                  </small>
                </div>
              </button>
            ))}
